...
int main() {
  ...
  if (retfour == NULL || retfour() != 4) {
    fprintf(stderr, "ERROR: dlsym returned original function, not wrapped from "
                    "libnum.so\n");
    had_error = -1;
  }
  ...
  if (test_retfive == NULL || test_retfive() != 5) {
    fprintf(stderr, "ERROR: call to return_five in libnum.so was not wrapped "
  ...
  if (retfour == NULL || retfour() != 4) {
    fprintf(stderr, "ERROR: call to return_four should be found in "
                    "RTLD_DEFAULT and return 4\n");
    had_error = -1;
  }
  ...
  if (test_retfive != NULL) {
    fprintf(stderr, "ERROR: call to test_return_five should NOT be found in "
                    "RTLD_DEFAULT\n");
    had_error = -1;
  }

...
PLUGIN_A="${abs_builddir}/.libs/libplugin_stat_a.so"
PLUGIN_B="${abs_builddir}/.libs/libplugin_stat_b.so"
TARGET="${abs_builddir}/app_test_static"
...
if [[ "$OUTPUT" == *"[AuditCore] WARNING: Diverging Chain Detected"* ]]; then
    echo "ERROR: Dynamic Dispatch warning unexpectedly found on a static sequence."
    exit 1
fi
